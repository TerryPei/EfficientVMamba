...
def direct_tokens(x, w=7, pos_direction=True):
    B, L, C = x.shape
    H = W = int(L ** 0.5)
    Hg = Wg = H // w
...
class DynMamba(nn.Module):
    def __init__(
        ...
        d_select = 192
        self.window_select = nn.Sequential(
            nn.Linear(self.d_model, d_select),
            nn.LayerNorm(d_select),
            nn.GELU(),
            nn.Linear(d_select, 2),
        )
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v2":
                ...
                H = W = int(seqlen ** 0.5)
                select = hidden_states.view(batch, H // 2, 2, W, -1).mean([2, 3])  # [B, H // 2, D]
                select = self.window_select(select)
                # select = F.gumbel_softmax(select, hard=True).view(batch, H // 2, 1, 1, -1)  # [B, H // 2, 1, 1, 2]
                select = F.softmax(select).view(batch, H // 2, 1, 1, -1)  # [B, H // 2, 1, 1, 2]
                select = select.repeat(1, 1, 2, W, 1).view(batch, seqlen, -1)
                xz_a = direct_tokens(xz, 2) * select[:, :, :1] + xz * select[:, :, 1:2]
                ...
                # out_b = reverse_tokens(out_b)

                out = reverse_tokens(out, 2) * select[:, :, :1] + out * select[:, :, 1:2]
                out_b = reverse_tokens(out_b) * select[:, :, :1] + out_b * select[:, :, 1:2]
