...
class CrossScan(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, x: torch.Tensor, window_size=2, index=0): # [B, C, H, W] -> [B, 4, H/w * W/w]
        B, C, H, W = x.shape
        ctx.shape = (B, C, H, W)
        ctx.window_size = window_size
        ctx.index = index

        if (W % window_size != 0) or (H % window_size != 0):
            x = F.pad(x, (0, window_size - H % window_size, 0, window_size - W % window_size))
            H, W = x.shape[2:]
        H = H // window_size
        W = W // window_size
        ...
        xs[:, 0] = x[:, :, pos[0][0]::window_size, pos[0][1]::window_size]
        xs[:, 1] = x[:, :, pos[1][0]::window_size, pos[1][1]::window_size].transpose(dim0=2, dim1=3)
        xs[:, 2] = x[:, :, pos[2][0]::window_size, pos[2][1]::window_size]
        xs[:, 3] = x[:, :, pos[3][0]::window_size, pos[3][1]::window_size].transpose(dim0=2, dim1=3)
    ...
    def backward(ctx, grad_xs: torch.Tensor): # [B, 4, H/w * W/w] -> [B, C, H, W]
        ...
        newH, newW = math.ceil(H / window_size), math.ceil(W / window_size)
        grad_x = grad_xs.new_empty((B, C, newH * window_size, newW * window_size))
        ...
        # 原Vmamba扫描四次时，每个位点的grad被累加了四次，因此输出为每个位点乘4.
        # 这里奇偶搭配每个位点的grad是独立 无累加
        xs = [grad_xs[:, 0], grad_xs[:, 1].transpose(dim0=2, dim1=3), grad_xs[:, 2], grad_xs[:, 3].transpose(dim0=2, dim1=3)]
        pos = ctx.pos

        grad_x[:, :, pos[0][0]::window_size, pos[0][1]::window_size] = xs[0]
        grad_x[:, :, pos[1][0]::window_size, pos[1][1]::window_size] = xs[1]
        grad_x[:, :, pos[2][0]::window_size, pos[2][1]::window_size] = xs[2]
        grad_x[:, :, pos[3][0]::window_size, pos[3][1]::window_size] = xs[3]

        if H != grad_x.shape[-2] or W != grad_x.shape[-1]:
            grad_x = grad_x[:, :, :H, :W]

        return grad_x, None, None # 分别对应forward的传入参数 x, window_size, (ctx.window_size参与计算了）



class CrossMerge(torch.autograd.Function): # [B, 4, C, H/w * W/w] -> [B, C, H*W]
    @staticmethod
    def forward(ctx, ys: torch.Tensor, ori_h, ori_w, window_size=2, index=0):
        B, K, C, H, W = ys.shape # B, 4, C, H/w * W/w
        ctx.shape = (H, W)
        ctx.ori_h = ori_h
        ctx.ori_w = ori_w
        ctx.window_size = window_size
        ...
        y = ys.new_empty((B, C, H * window_size, W * window_size))
        ...
        y0, y1, y2, y3 = ys[:, 0], ys[:, 1].transpose(dim0=2, dim1=3), ys[:, 2], ys[:, 3].transpose(dim0=2, dim1=3)

        y[:, :, pos[0][0]::window_size, pos[0][1]::window_size] = y0
        y[:, :, pos[1][0]::window_size, pos[1][1]::window_size] = y1
        y[:, :, pos[2][0]::window_size, pos[2][1]::window_size] = y2
        y[:, :, pos[3][0]::window_size, pos[3][1]::window_size] = y3
    ...
    def backward(ctx, grad_x: torch.Tensor): # [B, C, H*W] -> [B, 4, C, H/w * W/w]
        ...
        if (ctx.ori_w % window_size != 0) or (ctx.ori_h % window_size != 0):
            grad_x = F.pad(grad_x, (0, window_size - ctx.ori_h % window_size, 0, window_size - ctx.ori_w % window_size))

        pos = ctx.pos
        # print(x.shape, xs.shape, xs[:, 0].shape, x[:, :, ::window_size, ::window_size].shape) # torch.Size([16, 64, 56, 56]) torch.Size([16, 4, 64, 56, 56]) torch.Size([16, 64, 56, 56]) torch.Size([16, 64, 28, 28])

        grad_xs[:, 0] = grad_x[:, :, pos[0][0]::window_size, pos[0][1]::window_size]
        grad_xs[:, 1] = grad_x[:, :, pos[1][0]::window_size, pos[1][1]::window_size].transpose(dim0=2, dim1=3)
        grad_xs[:, 2] = grad_x[:, :, pos[2][0]::window_size, pos[2][1]::window_size]
        grad_xs[:, 3] = grad_x[:, :, pos[3][0]::window_size, pos[3][1]::window_size].transpose(dim0=2, dim1=3)
        
        return grad_xs, None, None, None, None
...
def cross_selective_scan(
    ...
    # H * W
    ori_h, ori_w = H, W
    xs = CrossScan.apply(x, window_size, index) # [B, C, H*W] -> [B, 4, C, H//w * W//w]
    # H//w * W//w
    H = math.ceil(H / window_size)
    W = math.ceil(W / window_size)
    ...
    y: torch.Tensor = CrossMerge.apply(ys, ori_h, ori_w, window_size, index) # [B, 4, C, H//w * W//w] -> [B, C, H*W]
...
class SS2D(nn.Module):
    ...
    def forward_corev2(self, x: torch.Tensor, nrows=-1, channel_first=False, window_size=2):
        nrows = 1
        if not channel_first:
            x = x.permute(0, 3, 1, 2).contiguous()
        ...
        x = cross_selective_scan(
            ...
            nrows=nrows, delta_softplus=True, window_size=window_size, index=self.index
        )
        if self.ssm_low_rank:
            x = self.out_rank(x)
        return x
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.d_conv > 1:
            ...
            if self.disable_z_act:
                ...
                xz = self.act(xz)
                x, z = xz.chunk(2, dim=-1) # (b, h, w, d)
        y = self.forward_core(x, channel_first=(self.d_conv > 1), window_size=self.window_size)
        y = y * z
...
class MobileVSSM(VSSM):
    def __init__(
        ...
        # Optionally, you can modify other layers or add new ones as per your design

    def forward(self, x: torch.Tensor):
        # Ensure to incorporate the modified downsampling step at the correct stage
        # print(x.shape) #[1, 3, 224, 224]
        x = self.patch_embed(x) #[1, 3, 224, 224] -> [1, 56, 56, 96]
        # print("forward: (self.patch_embed(x))", x.shape)
        # x = self.modified_downsampling(x)  # Apply the modified downsampling

        # Continue with the forward pass as in the original VSSM or with modifications
        for layer in self.layers:
            x = layer(x)
        # print("After x = layer(x), x.shape: ", x.shape)
        x = self.classifier(x)
        # print("After self.classifier(x): ", x.shape)
        return x


@register_model
def mobile_vssm_tiny(*args, drop_path_rate=0.1, **kwargs):
    return MobileVSSM(dims=[96, 192, 384, 768], depths=[2, 2, 9, 2], d_state=16, mlp_ratio=0, downsample_version='v1', drop_path_rate=drop_path_rate)

@register_model
def mobile_vssm_micro(*args, drop_path_rate=0.1, **kwargs):
    return MobileVSSM(dims=[64, 128, 256, 512], depths=[2, 2, 4, 2], d_state=16, mlp_ratio=0, downsample_version='v1', drop_path_rate=drop_path_rate)

@register_model
def mobile_vssm_nano(*args, drop_path_rate=0.1, **kwargs):
    return MobileVSSM(dims=[48, 96, 192, 384], depths=[2, 2, 4, 2], d_state=16, mlp_ratio=0, downsample_version='v1', drop_path_rate=drop_path_rate)