...
class CrossScan(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, x: torch.Tensor, window_size=2): # [B, C, H, W] -> [B, 4, H/w * W/w]
        ...
        if org_w % window_size != 0:
            pad_w = window_size - org_w % window_size
            x = F.pad(x, (0, pad_w, 0, 0))  
        W = x.shape[3]

        if org_h % window_size != 0:
            pad_h = window_size - org_h % window_size
        ...
        xs[:, 0] = x[:, :, ::window_size, ::window_size].contiguous().view(B, C, -1)
        xs[:, 1] = x.transpose(dim0=2, dim1=3)[:, :, ::window_size, 1::window_size].contiguous().view(B, C, -1)
        xs[:, 2] = x[:, :, ::window_size, 1::window_size].contiguous().view(B, C, -1)
        xs[:, 3] = x.transpose(dim0=2, dim1=3)[:, :, 1::window_size, 1::window_size].contiguous().view(B, C, -1)
    ...
    def backward(ctx, grad_xs: torch.Tensor): # [B, 4, H/w * W/w] -> [B, C, H, W]
        ...
        newH, newW = math.ceil(org_h / window_size), math.ceil(org_w / window_size)
        ...
        grad_x[:, :, ::window_size, ::window_size] = grad_xs[:, 0].reshape(B, C, newH, newW)
        grad_x[:, :, 1::window_size, ::window_size] = grad_xs[:, 1].reshape(B, C, newW, newH).transpose(dim0=2, dim1=3)
        grad_x[:, :, ::window_size, 1::window_size] = grad_xs[:, 2].reshape(B, C, newH, newW)
        grad_x[:, :, 1::window_size, 1::window_size] = grad_xs[:, 3].reshape(B, C, newW, newH).transpose(dim0=2, dim1=3)
...
class CrossMerge(torch.autograd.Function): # [B, 4, C, H/w * W/w] -> [B, C, H*W]
    @staticmethod
    def forward(ctx, ys: torch.Tensor, ori_h: int, ori_w: int, window_size=2):
        B, K, C, L = ys.shape
        H, W = math.ceil(ori_h / window_size), math.ceil(ori_w / window_size)
        ctx.shape = (H, W)
        ctx.ori_h = ori_h
        ctx.ori_w = ori_w
        ctx.window_size = window_size

        # print("ori_h, ori_w H W window_size: ", ori_h, ori_w, H, W, window_size)
        ...
        # print("After new_empty y.shape", y.shape, ys.shape)

        y[:, :, ::window_size, ::window_size] = ys[:, 0].reshape(B, C, H, W)
        y[:, :, 1::window_size, ::window_size] = ys[:, 1].reshape(B, C, W, H).transpose(dim0=2, dim1=3) 
        y[:, :, ::window_size, 1::window_size] = ys[:, 2].reshape(B, C, H, W)
        y[:, :, 1::window_size, 1::window_size] = ys[:, 3].reshape(B, C, W, H).transpose(dim0=2, dim1=3)
    ...
    def backward(ctx, grad_x: torch.Tensor): # [B, C, H*W] -> [B, 4, C, H/w * W/w]
        ...
        if ctx.ori_w % window_size != 0:
            pad_w = window_size - ctx.ori_w % window_size
            grad_x = F.pad(grad_x, (0, pad_w, 0, 0))  
        W = grad_x.shape[3]

        if ctx.ori_h % window_size != 0:
            pad_h = window_size - ctx.ori_h % window_size
        ...
        # print("grad_x.shape, grad_xs.shape", grad_x.shape, grad_xs.shape) # grad_x.shape, grad_xs.shape torch.Size([2, 1536, 26, 34]) torch.Size([2, 4, 1536, 884])
        
        grad_xs[:, 0] = grad_x[:, :, ::window_size, ::window_size].reshape(B, C, -1) #
        grad_xs[:, 1] = grad_x.transpose(dim0=2, dim1=3)[:, :, ::window_size, 1::window_size].reshape(B, C, -1)
        grad_xs[:, 2] = grad_x[:, :, ::window_size, 1::window_size].reshape(B, C, -1)
        grad_xs[:, 3] = grad_x.transpose(dim0=2, dim1=3)[:, :, 1::window_size, 1::window_size].reshape(B, C, -1)
...
def cross_selective_scan(
    ...
    xs = CrossScan.apply(x, window_size) # [B, C, H*W] -> [B, 4, C, H//w * W//w]
    # H//w * W//w
    ...
    # print(ys.shape, ori_h, ori_w, window_size, H, W)
    ori_h, ori_w = int(ori_h), int(ori_w)
    y = CrossMerge.apply(ys, ori_h, ori_w, window_size) # [B, 4, C, H//w * W//w] -> [B, C, H*W]
...
class SS2D(nn.Module):
    ...
    def forward_corev2(self, x: torch.Tensor, nrows=-1, channel_first=False, window_size=2):
        ...
        x = cross_selective_scan(
            ...
            nrows=nrows, delta_softplus=True, window_size=window_size
        )
        if self.ssm_low_rank:
            x = self.out_rank(x)
        return x
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.se = BiAttn(hidden_dim)  #SqueezeExcite(hidden_dim, hidden_dim // 8)
...
class Backbone_MobileVSSM(MobileVSSM):
