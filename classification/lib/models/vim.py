...
from .dyn_mamba_simple_search import DynMamba
# from .dyn_mamba_simple import DynMamba
...
def create_block(
    ...
    mixer_cls = partial(DynMamba, layer_idx=layer_idx, bimamba_type=bimamba_type, directions=directions, token_size=token_size, **ssm_cfg, **factory_kwargs)
    norm_cls = partial(
...
class VisionMamba(nn.Module):
    ...
    def flops(self, input_shape=(3, 224, 224)):
        supported_ops={
            ...
            "prim::PythonOp.LayerNormFn": None
        }

        model = copy.deepcopy(self)
        model.cuda().eval()

        input = torch.randn((1, *input_shape), device=next(model.parameters()).device)
        ...
        for layer in self.layers:
            ...
            # select
            # H = W = int(L ** 0.5)
            # flops += layer.mixer.d_model * layer.mixer.d_model * H // 2 + layer.mixer.d_model * 2 * H // 2
            """
            u: r(B D L)
            delta: r(B D L)
            A: r(D N)
        ...
        supported_ops={
            ...
            "prim::PythonOp.SelectiveScan": None,
            "prim::PythonOp.SelectiveScanFn": None,
        }

        input = torch.randn((1, *input_shape), device=next(model.parameters()).device)
        Gflops, unsupported = flop_count(model=self.cuda(), inputs=(input,), supported_ops=supported_ops)
        del model, input
        print(Gflops)
        print(unsupported)
        return sum(Gflops.values()) * 1e9
...
def vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_rope_also_residual(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=192, depth=20, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=True, if_rope_residual=True, bimamba_type="v2", **kwargs)
...
def local_vim_tiny_search(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=128, depth=20, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=True, if_rope_residual=True, bimamba_type="v2", **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
...
@register_model
def local_vim_tiny_searched(pretrained=False, **kwargs):
    directions = (
        ['h', 'v_flip', 'w7', 'w7_flip'],
    ...
    model = VisionMamba(
        patch_size=16, embed_dim=192, depth=20, rms_norm=False, residual_in_fp32=True, fused_add_norm=False, final_pool_type='mean', if_abs_pos_embed=True, if_rope=True, if_rope_residual=True, bimamba_type="v2", directions=directions, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
...
@register_model
def local_vim_small_searched(pretrained=False, **kwargs):
    directions = (
        ['h', 'v_flip', 'w7', 'w7_flip'],
        ['h_flip', 'w2_flip', 'w7', 'w7_flip'],
        ['h', 'h_flip', 'v', 'w7'],
...
def vim_small_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_rope_also_residual(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=384, depth=20, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=True, if_rope_residual=True, bimamba_type="v2", **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
...
def vim_base_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_rope_also_residual(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=768, depth=20, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=True, if_rope_residual=True, bimamba_type="v2", **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
