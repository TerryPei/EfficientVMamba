...
def direct_tokens(x, w=7, H=14, W=14, w_first=False):
    B, L, C = x.shape
    x = x.view(B, H, W, C)
...
def reverse_tokens(x, w=7, H=14, W=14, w_first=False):
    B, L, C = x.shape
    Hg, Wg = math.ceil(H / w), math.ceil(W / w)
    if H % w != 0 or W % w != 0:
        if w_first:
...
class MultiScan(nn.Module):
    ...
    def scan(self, x, direction='h'):
        ...
        if direction == 'h':
            ...
            return direct_tokens(x, w=2, H=H, W=W, w_first=False)
        elif direction == 'w2_flip':
            return direct_tokens(x, w=2, H=H, W=W, w_first=False).flip([1])
        elif direction == 'w7':
            return direct_tokens(x, w=7, H=H, W=W, w_first=False)
        elif direction == 'w7_flip':
            return direct_tokens(x, w=7, H=H, W=W, w_first=False).flip([1])
        else:
    ...
    def reverse(self, x, direction='h'):
        ...
        if direction == 'h':
            ...
            return reverse_tokens(x, w=2, H=H, W=W, w_first=False)
        elif direction == 'w2_flip':
            return reverse_tokens(x.flip([1]), w=2, H=H, W=W, w_first=False)
        elif direction == 'w7':
            return reverse_tokens(x, w=7, H=H, W=W, w_first=False)
        elif direction == 'w7_flip':
            return reverse_tokens(x.flip([1]), w=7, H=H, W=W, w_first=False)
        else:
            raise RuntimeError(f'Direction {direction} not found.')    
