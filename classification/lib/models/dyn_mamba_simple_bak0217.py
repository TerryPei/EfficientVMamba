...
def direct_tokens(x, w=2, pos_direction=True):
    B, L, C = x.shape
    H = W = int(L ** 0.5)
    Hg = Wg = H // w
    xs = x.chunk(4, dim=2)
    ...
    for idx, x in enumerate(xs):
        if idx % 2 == 0:
            x = x.view(B, Hg, w, Wg, w, -1).permute(0, 1, 3, 2, 4, 5).reshape(B, L, -1)
        new_xs.append(x)
    x = torch.cat(new_xs, 2)
    return x

def reverse_tokens(x, w=2):
    B, L, C = x.shape
    H = W = int(L ** 0.5)
    Hg = Wg = H // w
    xs = x.chunk(4, dim=2)
    ...
    for idx, x in enumerate(xs):
        if idx % 2 == 0:
            x = x.view(B, Hg, Wg, w, w, -1).permute(0, 1, 3, 2, 4, 5).reshape(B, L, -1)
        new_xs.append(x)
    x = torch.cat(new_xs, 2)
    return x
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v2":
                ...
                xz_a = rearrange(direct_tokens(xz, 2), "b l d -> b d l")
                xz_b = xz_a.flip([-1])
