...
def print_module_flops(module_flops_dict):
    # total_flops = sum(module_flops_dict.values())

    # 打印每个模块的 FLOPs
    print("Module FLOPs:")
    print("----------------------------------------------------------------")
    for module, flops in module_flops_dict.items():
        # 获取模块的类名和内存地址
        module_name = f"{module.__class__.__name__} ({hex(id(module))})"
        # 打印模块信息和其 FLOPs
        print(f"{module_name:60} : {flops:10.2f} FLOPs")


import logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

def test_flops(config):
    ...
    with torch.no_grad():
        ...
        # module_flops = model.module_flops()
        # for module_name, flops in module_flops.items():
        #     logging.info(f"Module Name: {module_name} | FLOPs: {flops} | GFLOPs: {flops / 1e9}")
        


if __name__ == '__main__':
    args, config = parse_option()
    # print("config.MODEL.VSSM.WINDOWSIZE: ", config.MODEL.VSSM.WINDOWSIZE)
    # print(config)
    test_flops(config)
    exit(0)

    # # log_file = f'logs/{config.MODEL.NAME}_flops.log' 
    # log_dir = 'logs'
    # log_file = os.path.join(log_dir, 'test_flops.log')
