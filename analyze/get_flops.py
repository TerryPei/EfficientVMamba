...
EfficientVSSM: nn.Module = build.vmamba_efficient.EfficientVSSM
...
def build_model_vssm(depths=[2, 2, 9, 2], embed_dim=[96, 192, 384, 768]):
...
def build_model_vssm_efficient(patch_size=4, depths=[2, 2, 9, 2], embed_dim=96, d_state=16, window_size=2):
    model = EfficientVSSM(patch_size=patch_size, depths=depths, dims=embed_dim, d_state=d_state, dt_rank="auto", ssm_ratio=2.0, mlp_ratio=0.0, downsample="v1", window_size=window_size)
...
def vssm_flops(core="fvcore"):
    ...
    build_vmamba_efficient = build_model_vssm_efficient
    _flops_count(build_vmamba_efficient(depths=[2, 2, 9, 2], embed_dim=96, window_size=2), input_shape=(3, 224, 224))

# def mobile_vssm_tiny(*args, drop_path_rate=0.1, **kwargs):
#     return EfficientVSSM(dims=[96, 192, 384, 768], depths=[2, 2, 9, 2], d_state=16, mlp_ratio=0, downsample_version='v1')


# def mobile_vssm_micro(*args, drop_path_rate=0.1, **kwargs):
#     return EfficientVSSM(dims=[64, 128, 256, 512], depths=[2, 2, 4, 2], d_state=16, mlp_ratio=0, downsample_version='v1')
